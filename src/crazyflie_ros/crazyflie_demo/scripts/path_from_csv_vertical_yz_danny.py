...
class Trajectory:
    def __init__(self, str_csv_file_name, str_veh_name):
        ...
        self.df = pd.read_csv('/home/stargazer/Dropbox/Data/'+str_csv_file_name , sep=',', header=None)
    ...
    def publishROS(self):
        ...
        # Move to the initial position #        
            ...
            self.pos_tgt_msg.linear.x = 0.0                             #(t-5)/5 * self.x_init
            self.pos_tgt_msg.linear.y = (t-5)/5 * self.x_init      #0.0 
            self.pos_tgt_msg.linear.z = (t-5)/5 * self.z_init + 0.5
            
        # Position Holding #        
        elif (t>=10) and (t < 15):
            self.pos_tgt_msg.linear.x = 0.0                #self.x_init
            self.pos_tgt_msg.linear.y = self.x_init      #0.0 
            self.pos_tgt_msg.linear.z = self.z_init +0.5
            
        # Path Flight # 
        elif (t>=15) and (t < 15 + self.total_path_time):
            t_path = t - 15
            self.pos_tgt_msg.linear.x = 0.0                                                                                                #np.interp(t_path, self.waypoint_data[0], self.waypoint_data[1])
            self.pos_tgt_msg.linear.y = np.interp(t_path, self.waypoint_data[0], self.waypoint_data[1])  #0.0
        ...
        # Position Hold before Landing # 
            ...
            self.pos_tgt_msg.linear.x = 0.0                  #self.x_term
            self.pos_tgt_msg.linear.y = self.x_term     #0.0
        ...
        # Landing
            ...
            self.pos_tgt_msg.linear.x = 0.0                  #self.x_term
            self.pos_tgt_msg.linear.y = self.x_term      #0.0
            self.pos_tgt_msg.linear.z = max(self.z_term + 0.5 - 0.6*t_landing,0) #min(max(self.z_term - 0.6*t_landing, 0.0),3.0)
...
if __name__ == '__main__':
    ...
    rate = rospy.Rate(100)   
    veh1 = Trajectory('IRL_path.csv','danny')    ### Initialize the class object
