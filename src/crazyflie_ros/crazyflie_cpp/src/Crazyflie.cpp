...
//YHJ begin The function below send packets thru radio.
void Crazyflie::sendViconPos(
  float x,
  float y,
  float z,
  float yaw)
{
  crtpViconPosRequest request(x,y,z,yaw);
  sendPacket((const uint8_t*)&request, sizeof(request));
}

void Crazyflie::sendViconKFPos(
  float x,
  float y,
  float z,
  float vx,
  ...
  float vz)
{
  crtpViconKFRequest request(x,y,z,vx,vy,vz);
  sendPacket((const uint8_t*)&request, sizeof(request));
}

void Crazyflie::sendPFcmd(
  float tgtX,
  float tgtY,
  float tgtZ,
  float tgtvX,
  ...
  float tgtvZ)
{
  crtpPFcmdRequest request(tgtX,tgtY,tgtZ,tgtvX,tgtvY,tgtvZ);
  sendPacket((const uint8_t*)&request, sizeof(request));
}
