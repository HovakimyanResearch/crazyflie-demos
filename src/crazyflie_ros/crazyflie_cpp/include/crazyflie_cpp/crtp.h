...
struct crtpViconPosRequest
{
  crtpViconPosRequest(
    ...
    float z,
    float yaw)
    : header(0x06, 0)  // This is the msg id for CrazyFlie(CF).
    , x(x)
    , y(y)
    , z(z)
    , yaw(yaw)
...
struct crtpViconKFRequest
{
  crtpViconKFRequest(
    ...
    float vy,
    float vz)
    : header(0x07, 0)  // This is the msg id for CrazyFlie(CF).
    , x(x)
    , y(y)
    , z(z)
    , vx(vx)
...
struct crtpPFcmdRequest
{
  crtpPFcmdRequest(
    ...
    float tgtvY,
    float tgtvZ)
    : header(0x08, 0)  // This is the msg id for CrazyFlie(CF).
    , tgtX(tgtX)
    , tgtY(tgtY)
    , tgtZ(tgtZ)
    , tgtvX(tgtvX)
